...
TEMPORAL_ROOT=temporal/test-1
SPATIAL_ROOT=spatial/test-1
...
SKIP=0.2
FFMPEG_FILTER="\
[0:v]scale=height=512:width=-2,${drawtext_abstract_filter}:text='Source'[video];\
[1:v]${drawtext_abstract_filter}:text='Flow U'[flow_u];\
[2:v]${drawtext_abstract_filter}:text='Flow V',$timestamp_filter[flow_v];\
